...
class NerfLoss(torch.nn.Module):
    ...
    def forward(self, inputs, targets):
        loss_dict = {}
        loss_dict['coarse_color'] = self.loss(inputs['rgb_coarse'], targets) # RGB MSE값들 loss 
...
def uncertainty_aware_loss(loss_dict, inputs, gt_rgb, typ, beta_min=0.05):
    # SAT NeRF의 Beta Loss
    # 출력 : loss_dict -> color와 log(beta)
    beta = torch.sum(inputs[f'weights_{typ}'].unsqueeze(-1) * inputs['beta_coarse'], -2) + beta_min # lamda + beta + beta_min
    loss_dict[f'{typ}_color'] = ((inputs[f'rgb_{typ}'] - gt_rgb) ** 2 / (2 * beta ** 2)).mean() # L_RGB using beta
    loss_dict[f'{typ}_logbeta'] = (3 + torch.log(beta).mean()) / 2  # +3 to make c_b positive since beta_min = 0.05 :: log beta
    return loss_dict

def solar_correction(loss_dict, inputs, typ, lambda_sc=0.05):
    # computes the solar correction terms defined in Shadow NeRF and adds them to the dictionary of losses :: L_sc
    sun_sc = inputs[f'sun_sc_{typ}'].squeeze()
    term2 = torch.sum(torch.square(inputs[f'transparency_sc_{typ}'].detach() - sun_sc), -1) # Ti-S 의 MSE
    term3 = 1 - torch.sum(inputs[f'weights_sc_{typ}'].detach() * sun_sc, -1) # 우측 부분.
...
class SNerfLoss(torch.nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        if self.lambda_sc > 0:
            loss_dict = solar_correction(loss_dict, inputs, typ, self.lambda_sc) #SC loss
        if 'rgb_fine' in inputs:
            typ = 'fine'
            loss_dict[f'{typ}_color'] = self.loss(inputs[f'rgb_{typ}'], targets)
            if self.lambda_sc > 0:
...
class SatNerfLoss(torch.nn.Module):
    ...
    def forward(self, inputs, targets):
        loss_dict = {}
        typ = 'coarse'
        loss_dict = uncertainty_aware_loss(loss_dict, inputs, targets, typ) # L_RGB
        if self.lambda_sc > 0:
            loss_dict = solar_correction(loss_dict, inputs, typ, self.lambda_sc) # L_SC
        if 'rgb_fine' in inputs:
            typ = 'fine'
            loss_dict = uncertainty_aware_loss(loss_dict, inputs, targets, typ) # L_RGB
            if self.lambda_sc > 0:
